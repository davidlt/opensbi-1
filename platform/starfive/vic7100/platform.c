...
static struct sbi_system_reset_device vic7100_reset = {
	.name = "vic7100_reset",
	.system_reset_check = vic7100_system_reset_check,
	.system_reset = vic7100_system_reset
};

static int vic7100_early_init(bool cold_boot)
{
	if (cold_boot) {
		sbi_system_reset_set_device(&vic7100_reset);
	} else {
		/* Store to other core */
	}
...
static int vic7100_domains_init(void)
{
	...
	for (i = 0; i < ROOT_END_REGION; i++) {
		rc = sbi_domain_root_add_memregion(&vic7100_root_memregs[i]);
		if (rc)
			return rc;
	}
